...
const navItems = computed(() => {
  ...
  if (!isAdmin.value) {
    items.push({ name: 'Profile', path: '/profile', icon: 'profile' })
  }
  ...
  <aside class="w-64 bg-sidebar text-white flex flex-col shrink-0">
    ...
    <nav class="flex-1 p-4 space-y-1">
      ...
      >
        ...
        <svg v-if="item.icon === 'profile'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-4 border-t border-white/10">
      <div class="flex items-center gap-3 px-4 py-3">
        <div class="w-9 h-9 rounded-full flex items-center justify-center text-xs font-semibold shrink-0"
          :class="isAdmin ? 'bg-accent/20 text-accent' : 'bg-primary/20 text-primary-light'">
        ...
        <button @click="logout" class="p-2 text-white/40 hover:text-white hover:bg-sidebar-hover rounded-lg transition-colors" title="Sign out">
